...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(
      () => route.path,
      (vl) => {
        // 切换路由时，设置当前路由 未找到路由匹配 home
        const matchedRoute = getValueByKeys(getValueByKeys(router.currentRoute.value.meta, "matched", [])[0], "path", "") || "/home";
...
<template>
  ...
  >
    <sidebar-menus-items :className="state.popClassName" :menus="state.menus" :hiddenIndex="state.hiddenIndex" />
  </el-menu>
</template>
